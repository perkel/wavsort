...
Version 0.3.3. Adding back much of what was removed to port this to python 3
Adding syllable segmentation.
Added FF analysis
...
class GraphFrame(wx.Frame):
    ...
    def open_file(self, path, batch=False):
        ...
        if not batch:
            ...
            if lp_cutoff:
                ...
                maxfreq = None
            xextent = (0, self.times[-1])  # this seems to be the convention - range from zero to time of last sample, even though it's probably not used
    ...
    def create_main_panel(self):
        ...
        self.batch_button = wx.Button(self.panel, -1, "Batch sort", style=wx.ALIGN_BOTTOM)
        self.Bind(wx.EVT_BUTTON, self.on_batch_calc, self.batch_button)
        ...
        self.hbox1.Add(self.batch_button, 0)
    ...
    def plot(self):
        ...
        self.axes.plot(self.times, amplitude_envelope, 'b')
    ...
    def on_batch(self, event):
        if not self.trained_folder and self.use_ml.GetValue():
            self.ErrorDlg('Must train first')
            return
        ...
        wildcard = 'Output file (*.csv)|*.csv|All files (*.*)|*.*'
        dlg = wx.FileDialog(app.frame, message='Save data as...', defaultDir=self.currFolder,
                            defaultFile='.csv', wildcard=wildcard, style=wx.SAVE | wx.OVERWRITE_PROMPT)
        if dlg.ShowModal() != wx.ID_OK:
            return
        writeFileName = dlg.GetPath()
        dlg.Destroy()

        try:
            out_file = open(writeFileName, 'w')
        except IOError:
            estr = "I/O error"
            if errno == 13:
        ...
        out_file.write('Folder: %s\n' % self.currFolder)
        out_file.write('Goodness-of-pitch threshold %%: %s\n' % self.good_thr_pcnt.GetValue())
        out_file.write('Max pitch (kHz): %s\n' % self.max_pitch_freq_khz.GetValue())
        out_file.write('Threshold %%: %s\n' % self.thr_pcnt.GetValue())
        out_file.write('Shoulder (ms): %s\n' % self.shoulder.GetValue())
        out_file.write('Silence factor: %s\n' % self.silence_factor.GetValue())
        out_file.write('High freq cutoff (kHz): %s\n' % self.low_pass_f.GetValue())
        out_file.write('Num syllables: %s\n' % self.num_syllables.GetValue())
        out_file.write('\nfile_num, filename, syll#, start, dur, n_stacked, FF1, FF2, FF3\n')

        for self.thisFile, dataFile in enumerate(self.fileList):  # loop on files
            self.open_file(dataFile, batch=True)
            self.file_num.SetValue(str(self.thisFile))
            wx.Yield()
            for syll_num, (time, dur) in enumerate(self.syllable_times):
                n_stacked = 0
                FF1 = 0.0
                FF2 = 0.0
                FF3 = 0.0
                # for stack_start_t, stack_dur, pitches in self.stacks:
                #    # do we have any harmonic stacks in this syll?
                #    if time <= stack_start_t <= time+dur:
                #        FF = sum(pitches)/len(pitches)
                #        n_stacked += 1
                #        if n_stacked == 1:
                #            FF1 = FF
                #        elif n_stacked == 2:
                #            FF2 = FF
                #        else:
                #            # unexpected, but user will see large n_stacked
                #            print '********MORE THAN 2 STACKED HARMONICS IN SYLLABLE'
                for i in self.stacks:
                    ...
                    if time <= stack_start_t <= time + dur:
                        ...
                        if n_stacked == 1:
                            ...
                            # unexpected, but user will see large n_stacked
                            print('********MORE THAN 3 STACKED HARMONICS IN SYLLABLE')
                out_file.write('%d, "%s", %d, %.3f, %.3f, %d, %.3f, %.3f, %.3f\n' % (
                    self.thisFile, os.path.basename(dataFile),
                    syll_num + 1, time, dur, n_stacked, FF1, FF2, FF3))
        out_file.close()

    def on_batch_calc(self, event):
        # if not self.trained_folder and self.use_ml.GetValue():
        #     self.ErrorDlg('Must train first')
        #     return
        ...
        wildcard = 'Output file (*.csv)|*.csv|All files (*.*)|*.*'
        dlg = wx.FileDialog(frame, message='Save data as...', defaultDir=self.currFolder,
                            defaultFile='.csv', wildcard=wildcard, style=wx.FD_SAVE | wx.FD_OVERWRITE_PROMPT)
        if dlg.ShowModal() != wx.ID_OK:
            return
        writeFileName = dlg.GetPath()
        dlg.Destroy()
        ...
        out_file.write('Folder: %s\n' % self.currFolder)
        out_file.write('Duration threshold: %s\n' % self.durationThresh)
        # out_file.write('Goodness-of-pitch threshold %%: %s\n' % self.good_thr_pcnt.GetValue())
        # out_file.write('Max pitch (kHz): %s\n' % self.max_pitch_freq_khz.GetValue())
        # out_file.write('Threshold %%: %s\n' % self.thr_pcnt.GetValue())
        # out_file.write('Shoulder (ms): %s\n' % self.shoulder.GetValue())
        ...
        # out_file.write('High freq cutoff (kHz): %s\n' % self.low_pass_f.GetValue())
        # out_file.write('Num syllables: %s\n' % self.num_syllables.GetValue())
        out_file.write('\nfile_num, filename, Max envelope, Fraction above thresh\n')
        ...
        for self.thisFile, dataFile in enumerate(self.fileList):  # loop on files
            ...
            # Calculate duration over, say 5% of baseline
            minval = np.min(amplitude_envelope)
            maxval = np.max(amplitude_envelope)
            threshval = minval + self.durationThresh*(maxval-minval)
            aboveThresh = np.argwhere(self.data > threshval)
            fracAboveThresh = len(aboveThresh)/len(self.data)

            # Criteria for sorting
            maxBad = 0.15 # maximum fraction below threshold to exclude
            minGood = 0.17 # minimum fraction above threshold to include
            envThresh = 250 # minimum peak envelope threshold to include
            if (fracAboveThresh > minGood) and (maxval>envThresh):
                # move to include
                move(dataFile, 'include')
            elif (fracAboveThresh < maxBad) or (maxval < envThresh):
                # move to exclude
                move(dataFile, 'exclude')
            else:
                # move to uncertain
                move(dataFile, 'uncertain')

            out_file.write('%d, "%s", %.3f, %.3f\n' % (self.thisFile, os.path.basename(dataFile), maxval, fracAboveThresh))
    ...
    def calcFeatures(self):
        self.nFiles = self.getFileList()
        self.nParams = 3
        self.fileVals = np.array(nFiles, nParams)
        if (debug):
            print("in sort. nf = ", self.nFiles)
            print("file list is ", self.fileList)
        for dataFile in self.fileList:
            ...
            self.axes.clear()
            self.canvas.draw()
            # Calculate values and put them in fileVals
